...
export const authService = {
    ...
    async registerVerify(payload) {
        ...
        if (!pending)
            throw makeError('No pending registration found. Please start over.', 400);
    ...
    // ── OTP Login ─────────────────────────────────────────────────
    async loginSendOtp(email) {
        const user = await prisma.user.findUnique({ where: { email } });
        if (!user)
            throw makeError('No account found with that email address.', 404);
    ...
    async loginVerifyOtp(payload) {
        ...
        if (!valid)
            throw makeError('Invalid or expired OTP', 401);
        const user = await prisma.user.findUnique({ where: { email: payload.email } });
        if (!user)
