...
export const emailService = {
  async sendRegistrationOtp(email: string, name: string, otp: string) {
    const html = buildOtpHtml(
      ...
      'Welcome to BasantiShop! Use the code below to verify your email address and complete your registration.',
      otp,
      10
    )
  ...
  async sendLoginOtp(email: string, otp: string) {
    const html = buildOtpHtml(
      'Your login code',
      'Use the code below to sign in to your BasantiShop account.',
      otp,
      10
    )
  ...
  async sendPasswordResetOtp(email: string, otp: string) {
    const html = buildOtpHtml(
      'Reset your password',
      "We received a request to reset your BasantiShop password. Use the code below to set a new password. If you didn't request this, ignore this email.",
      otp,
      15
    )
