...
const imageUrlSchemaCheck = (url: string) => {
  try {
    const u = new URL(url)
...
async function ratingStatsMap(productIds: string[]) {
  if (!productIds.length) return new Map<string, { averageRating: number; ratingCount: number }>()
  ...
  const m = new Map<string, { averageRating: number; ratingCount: number }>()
  for (const r of rows) {
    m.set(r.productId, {
...
function productPublicDto(
  ...
  userRating?: { myRating: number | null }
) {
  return {
    id: p.id,
    slug: p.slug,
...
export const productService = {
  ...
  async list(params: {
    ...
    if (typeof params.minPrice === 'number' || typeof params.maxPrice === 'number') {
      where.sellingPrice = {}
      if (typeof params.minPrice === 'number') where.sellingPrice.gte = new Prisma.Decimal(params.minPrice)
      if (typeof params.maxPrice === 'number') where.sellingPrice.lte = new Prisma.Decimal(params.maxPrice)
    }

    let orderBy: Prisma.ProductOrderByWithRelationInput = { createdAt: 'desc' }
    if (params.sortBy === 'price-asc') orderBy = { sellingPrice: 'asc' }
    if (params.sortBy === 'price-desc') orderBy = { sellingPrice: 'desc' }
    ...
    return {
      data: products.map((p) =>
        productPublicDto(
          p,
          stats.get(p.id),
          params.userId ? { myRating: myRatings.get(p.id) ?? null } : undefined
        )
      ),
      total,
      page,
  ...
  async update(
    ...
    if (payload.images) this.validateImageUrls(payload.images)
    ...
    if (payload.originalPrice !== undefined) data.originalPrice = new Prisma.Decimal(payload.originalPrice)
    if (payload.sellingPrice !== undefined) data.sellingPrice = new Prisma.Decimal(payload.sellingPrice)
  ...
  async upsertRating(userId: string, productId: string, rating: number) {
